...
from . import TestCase, es, helpers as h
...
class TestAnnotation(TestCase):
    ...
    def test_save_assert_refresh(self):
        ...
        es.index = 'foo'
        a.save(es)
        args, kwargs = es.conn.index.call_args
    ...
    def test_delete(self):
        ...
        newann = Annotation.fetch(es, 1)
        newann.delete(es)

        noann = Annotation.fetch(es, 1)
    ...
    def test_basics(self):
        ...
        ann = Annotation.fetch(es, ann.id)
    ...
    def test_search(self):
        ...
        res = Annotation.search(es)
        assert_equal(len(res), 3)

        # ordering (most recent first)
        assert_equal(res[0]['text'], uri2)

        res = Annotation.count(es)
        assert_equal(res, 3)

        res = Annotation.search(es, limit=1)
        assert_equal(len(res), 1)
        res = Annotation.count(es, limit=1)
        assert_equal(res, 3)

        res = Annotation.search(es, query={'uri':uri1})
        assert_equal(len(res), 2)
        assert_equal(res[0]['uri'], uri1)
        assert_equal(res[0]['id'], anno2.id)

        res = Annotation.search(es, query={'user':user1})
        assert_equal(len(res), 2)
        assert_equal(res[0]['user'], user1)
        assert_equal(res[0]['id'], anno3.id)

        res = Annotation.search(es, query={'user':user1, 'uri':uri2})
        assert_equal(len(res), 1)
        assert_equal(res[0]['user'], user1)
        assert_equal(res[0]['id'], anno3.id)

        res = Annotation.count(es, query={'user':user1, 'uri':uri2})
        assert_equal(res, 1)
    ...
    def test_search_permissions_null(self):
        ...
        user = h.MockUser('bob')
        res = Annotation.search(es, user=user)
        assert_equal(len(res), 0)
